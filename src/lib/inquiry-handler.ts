...
async function readBounded(request: Request): Promise<Uint8Array> {
  ...
  try {
    while (true) {
      ...
      if (size > inquiryConfig.maxBodyBytes) {
        await reader.cancel();
...
export async function handleInquiry(request: Request) {
  ...
  if (origin && origin !== new URL(request.url).origin)
    return reply({ ok: false, message: "Отправьте форму со страницы сайта." }, 403);
  if (request.headers.get("sec-fetch-site") === "cross-site")
    return reply({ ok: false, message: "Межсайтовая отправка запрещена." }, 403);
  const contentType = request.headers.get("content-type") ?? "";
  if (!contentType.startsWith("multipart/form-data;"))
    return reply({ ok: false, message: "Неверный формат запроса." }, 415);
  if (Number(request.headers.get("content-length")) > inquiryConfig.maxBodyBytes)
    return reply(
      { ok: false, message: "Запрос слишком большой. Максимальный размер файла — 5 МБ." },
      413,
    );
  let form: FormData;
  try {
    ...
    return reply(
      {
        ok: false,
        message: oversized ? "Максимальный размер файла — 5 МБ." : "Не удалось прочитать форму.",
      },
      oversized ? 413 : 400,
    );
  }
  ...
  for (const [key, value] of form.entries()) {
    if (form.getAll(key).length !== 1)
      return reply({ ok: false, message: "Повторяющиеся поля формы." }, 400);
  ...
  if (
    ...
    return reply(
      { ok: false, message: "Не удалось проверить форму. Обновите страницу и повторите ввод." },
      400,
    );
  const errors = validateInquiry(values, file);
  if (Object.keys(errors).length)
    return reply({ ok: false, message: "Проверьте поля формы.", errors }, 422);
  if (file && file.size > 0 && !(await validFileSignature(file)))
    return reply(
      {
        ok: false,
        message: "Проверьте вложение.",
        errors: { file: "Содержимое файла не соответствует расширению." },
      },
      422,
    );
