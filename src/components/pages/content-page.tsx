...
export function ContentPage({ slug }: { slug: string }) {
  ...
  const crumbs = [
    ...(parent ? [{ label: parent.label, href: hrefFor(parent.slug) }] : []),
    { label: page.label, href: hrefFor(page.slug) },
  ];
  return (
    <main id="main">
      <Container>
      ...
      <Hero title={page.h1} intro={page.intro} eyebrow={parent?.label ?? "Троллейный шинопровод"}>
        {page.category !== "index" && slug !== "podbor-oborudovaniya" && (
          <div className="mt-7">
      ...
      {slug === "produkciya" && (
        <Section title="Основные типы систем">
          <div className="grid gap-5 lg:grid-cols-3">
            {products.map((item, i) => (
              <ProductDirectionCard key={item.slug} page={item} number={i + 1} />
            ))}
          </div>
        </Section>
      )}
      {slug === "resheniya" && (
        <Section title="Выберите ваше оборудование">
          <div className="grid gap-x-12 lg:grid-cols-2">
            {solutions.map((item, i) => (
              <SolutionCard key={item.slug} page={item} number={i + 1} />
            ))}
          </div>
        </Section>
      )}
      {slug === "otrasli" && (
        <Section title="Условия применения по отраслям">
          <div className="grid gap-5 sm:grid-cols-2">
            {industries.map((item) => (
              <IndustryCard key={item.slug} page={item} />
            ))}
          </div>
        </Section>
      )}
      {page.sections.length > 0 && (
        <Section>
          <div
            className={`grid items-start gap-10 ${page.media ? "lg:grid-cols-[1fr_.8fr]" : "lg:grid-cols-[.35fr_1fr]"}`}
          >
            ...
            {page.media && (
              <aside className="space-y-6">
                <Media asset={page.media} large />
                <div className="border border-border bg-surface p-6">
                  <p className="eyebrow mb-4">Для точного подбора</p>
                  <p className="text-sm leading-7 text-muted-foreground">
                    Схема поясняет принцип работы. Геометрия, состав и характеристики определяются
                    конкретным исполнением и проектом.
                  </p>
                  <div className="mt-5">
      ...
      {(slug === "produkciya" || slug === "resheniya" || slug === "resheniya/mostovye-krany") && (
        <Comparison />
      )}
      {(slug === "resheniya" || slug === "podbor-oborudovaniya" || slug === "o-kompanii") && (
        <ProcessSteps />
      )}
      ...
      {(page.category === "product" ||
        slug === "produkciya" ||
        slug === "brands/anneng" ||
        slug === "kontakty") && <CatalogCTA shopKey={page.shopKey} />}
      {page.form && (
        <Section
          id="podbor"
          title={
        ...
        >
          <div className="max-w-4xl">
            <ContactForm extended={slug === "podbor-oborudovaniya"} />
          </div>
        </Section>
      )}
