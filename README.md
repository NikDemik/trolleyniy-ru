...
Счётчик загружается только после явного разрешения в cookie-баннере. Укажите ID счётчика
в окружении сборки и заново соберите приложение:

```bash
NEXT_PUBLIC_YANDEX_METRIKA_ID=12345678
```

Без переменной баннер и сохранение выбора работают, но запросы к Метрике не выполняются.
Форма заявки помечена `ym-hide-content` и целиком исключена из записи Вебвизором.
...
В `src/config/company.ts` заполнены контактные данные, реквизиты, документы,
юридические ссылки и `commercialTerms`. Условия оплаты, доставки и гарантии сверены
с публичными страницами B2B-каталога ANNENG; для корпоративного сайта сохранены
оговорки о согласовании конкретной поставки в счёте, спецификации или договоре.
Единый гарантийный срок не заявляется: он определяется документами на выбранное
оборудование и документами конкретной поставки.
